...
Unfortunately, it was not as simple as passing the data directly from one application to the other. The user is required to perform a transformation of the CSV-esque data into XML. To perform this transformation, any multi-line messages has to be reverted to a single line with HTML break tags. The file must then be put through a Perl script, which Mario had developed. At this stage, I had to perform some minor modifications to get it to work. My version of the script can be downloaded "here":http://www.lewisoaten.com/files/2009-01-20-mobile-data-preservation/PIM2smsxml.pl.
